...
def get_embedding_config(embed_info: dict) -> dict:
    ...
    try:
        if embed_info:
            ...
            # 安全地获取 api_key，支持直接值或环境变量名
            api_key_value = embed_info.get("api_key")
            if api_key_value:
                config_dict["api_key"] = os.getenv(api_key_value, api_key_value)
            else:
                # 如果没有 api_key，尝试从环境变量获取默认值
                config_dict["api_key"] = os.getenv("SILICONFLOW_API_KEY") or os.getenv("OPENAI_API_KEY")
